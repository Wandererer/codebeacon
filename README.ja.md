...
- **tree-sitter ベース** — 正規表現ではなく構造的 AST パース；言語別オプションインストールで軽量を維持
...
pip install codebeacon[all]   # すべての言語パーサー
...
必要な言語パーサーだけをインストールできます：

```bash
pip install codebeacon                      # コアのみ (Python プロジェクトはそのまま動作)
pip install codebeacon[java,python,js]      # 特定の言語サポート
pip install codebeacon[all]                 # すべての言語パーサー
pip install codebeacon[all,cluster]         # + Leiden コミュニティ検出 (graspologic)
```

利用可能な言語 extras: `java`、`kotlin`、`python`、`js`、`go`、`ruby`、`php`、`csharp`、`rust`、`swift`、`html`、`svelte`
