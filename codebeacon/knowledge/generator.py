...
# Ownership markers for ``KNOWLEDGE.md``. A scan may regenerate the map only
# when it recognises its own output — a hand-written KNOWLEDGE.md must never be
# clobbered by an unattended rebuild (see ``link.reapply_knowledge``).
KNOWLEDGE_MAP_HEADER = "# Knowledge Map — "
KNOWLEDGE_MAP_FOOTER = "_Generated by codebeacon"

# Markers for codebeacon's OTHER generated markdown — the context maps written
# into the repo root (CLAUDE.md / AGENTS.md / .cursorrules). Mirrored here
# rather than imported from ``contextmap.generator`` to keep the knowledge
# scanner free of a dependency on the context-map writer.
_CONTEXTMAP_BLOCK_START = "<!-- codebeacon:start -->"
_CONTEXTMAP_FOOTER = "_Generated by [codebeacon]"
SKIP_DIRS = {
...
class Note:
    ...
    # Frontmatter keys beyond the ones promoted to their own field above
    # (status, owner, epic, …), bounded and sanitized. Carried onto the graph
    # node so a vault's own conventions survive into beacon.json.
...
class KnowledgeResult:
    ...
    # Where KNOWLEDGE.md was *asked* to go, recorded even when ``write_output``
    # suppressed the write, so an overlay rebuild can find the same location.
    output_dir: str = ""
...
def build_knowledge_map(
    ...
    *,
    ...
    Args:
        ...
        write_output: when False the notes are classified but no file is
                    written and ``output_path`` stays ``None``. Used by the
                    scan-side overlay rebuild when a ``KNOWLEDGE.md`` exists
                    that codebeacon did not author.
    """
    ...
    result = KnowledgeResult(root=str(root_path), notes=notes, output_dir=str(output_path))

    if write_output:
        result.output_path = _write_knowledge_map(output_path / "KNOWLEDGE.md", result)
    return result


# The generation stamp in the footer, used to re-render a prior map for
# comparison. Anchored to the footer marker so prose containing a date cannot
# be mistaken for it.
_STAMP_RE = re.compile(
    rf"^{re.escape(KNOWLEDGE_MAP_FOOTER)} · (\d{{4}}-\d{{2}}-\d{{2}})_$", re.MULTILINE
)


def _write_knowledge_map(out_file: Path, result: KnowledgeResult) -> Path:
    """Write ``KNOWLEDGE.md``, but only when the vault actually changed.

    ``.codebeacon``'s outputs are committed, and the overlay is now rebuilt on
    every scan (R2), so an unconditional write would move this file's mtime on
    every run — waking Obsidian's indexer, sync clients, and codebeacon's own
    watch mode over a document whose only difference is its date stamp
    (GI-3060). A map that is identical apart from that stamp is left untouched.
    """
    from codebeacon.common.io import write_text_if_changed

    try:
        previous = out_file.read_text(encoding="utf-8")
    except (OSError, UnicodeDecodeError, ValueError):
        previous = None

    if previous is not None:
        stamp = _STAMP_RE.search(previous)
        if stamp and _render_markdown(result, today=stamp.group(1)) == previous:
            return out_file  # nothing but the date would have moved

    write_text_if_changed(out_file, _render_markdown(result))
    return out_file
...
def _classify(text: str, rel_path: str, filename: str) -> Optional[Note]:
    ...
    if not text.strip():
        return None
    # Never ingest our own output. KNOWLEDGE.md and the context maps
    # (CLAUDE.md / AGENTS.md / .cursorrules) are all written INTO the scanned
    # tree, so without this a re-run classifies the previous run's artefacts as
    # notes. ``SKIP_DIRS`` already excludes ``.codebeacon``/``.codesight`` for
    # the same reason; these files just happen to live at the repo root. It
    # matters more now that the overlay is rebuilt on every scan
    # (``link.reapply_knowledge``) rather than only on an explicit command.
    if is_codebeacon_artifact(text):
        return None
    ...
    return Note(
        ...
        frontmatter=_passthrough_frontmatter(frontmatter),
    )


# Frontmatter keys already promoted to a dedicated ``Note`` field — carrying
# them again under ``frontmatter`` would just duplicate data on the node.
_PROMOTED_FRONTMATTER_KEYS = frozenset({
    "title", "summary", "description", "tags",
    "date", "created", "updated", "decision-date",
    "type", "category",
})

# A vault's own conventions are worth carrying, an unbounded key space is not.
MAX_FRONTMATTER_KEYS = 20
MAX_FRONTMATTER_VALUE_CHARS = 200


def _passthrough_frontmatter(frontmatter: dict[str, str]) -> dict[str, str]:
    """Return the non-promoted frontmatter keys, sanitized and bounded.

    ``_parse_simple_yaml`` already gives a flat scalar view, so the only work
    here is dropping what is already promoted, running keys and values through
    ``sanitize_label`` (frontmatter is user text that ends up in YAML output
    and MCP responses), and capping both the key count and value length. Keys
    are taken in sorted order so the cap is deterministic rather than
    ...
    for key in sorted(frontmatter):
        if key in _PROMOTED_FRONTMATTER_KEYS:
            continue
        clean_key = sanitize_label(key)
        if not clean_key:
            continue
        value = frontmatter[key]
        if not isinstance(value, str):
            continue
        clean_value = sanitize_label(value)[:MAX_FRONTMATTER_VALUE_CHARS]
        if not clean_value:
            continue
        out[clean_key] = clean_value
        if len(out) >= MAX_FRONTMATTER_KEYS:
            break
    return out
...
def _render_markdown(result: KnowledgeResult, today: Optional[str] = None) -> str:
    """Render KNOWLEDGE.md. ``today`` overrides the generation stamp.

    The override exists so a rebuild can re-render with the PREVIOUS run's date
    and compare: if the two match, nothing about the vault changed and the file
    is left alone rather than rewritten with a new date. Same reasoning as
    ``built_at_ts`` in graph/write.py — a date stamp should not be the only
    reason a committed artifact's mtime moves.
    """
    ...
    out: list[str] = []
    out.append(f"{KNOWLEDGE_MAP_HEADER}{project_name}")
    parts = [f"{total} notes"]
        ...
        for n in bucket:
            ...
            if n.summary and n.summary != n.title:
                ...
                if len(summary) > 140:
                    summary = summary[:140] + "…"
                line += f" — {summary}"
            # Authored [[wikilinks]] are the vault's own cross-note structure;
            # showing them here gives the parsed data a visible consumer next
            # to the graph edges knowledge/link.py materialises from them.
            if n.backlinks:
                shown = ", ".join(f"[[{b}]]" for b in n.backlinks[:5])
                if len(n.backlinks) > 5:
                    shown += f", +{len(n.backlinks) - 5} more"
                line += f" — links: {shown}"
            out.append(line)

    out.append("")
    out.append("---")
    out.append(f"{KNOWLEDGE_MAP_FOOTER} · {today}_")
    out.append("")
    return "\n".join(out)


def is_generated_knowledge_map(text: str) -> bool:
    """True when ``text`` looks like a ``KNOWLEDGE.md`` codebeacon itself wrote.

    Both the header and the footer must be present, so a note that merely
    mentions codebeacon in prose is not mistaken for our own output.

    Deliberately STRICTER than :func:`is_codebeacon_artifact`, because the two
    answer different questions: this one gates *overwriting* a file on disk, so
    a false positive destroys the user's work. The other gates *ingesting* a
    file, where a false positive costs only a missing note.
    """
    if not text:
        return False
    head = text[:4096]
    return KNOWLEDGE_MAP_HEADER in head and KNOWLEDGE_MAP_FOOTER in text


def is_codebeacon_artifact(text: str) -> bool:
    """True when ``text`` is markdown codebeacon generated (any writer).

    Covers ``KNOWLEDGE.md`` and the context maps. Used to keep our own output
    out of the note corpus; see :func:`_classify`.
    """
    if not text:
        return False
    return (
        is_generated_knowledge_map(text)
        or _CONTEXTMAP_BLOCK_START in text
        or _CONTEXTMAP_FOOTER in text
    )
