...
from codebeacon.common.safety import defang_model_tokens
...
def _after_footer(text: str) -> str:
    """Return whatever follows the last generated-footer line in *text*.

    Used to bound a codebeacon block whose end marker is missing: the footer
    (``_Generated by [codebeacon]``) is the last line every generated block
    emits, so anything below it is the user's. No footer ⇒ no safe boundary ⇒
    the whole text is treated as the user's and kept.
    """
    lines = text.splitlines()
    footer_idx = None
    for i, ln in enumerate(lines):
        if ln.strip().startswith(_FOOTER_MARKER):
            footer_idx = i
    if footer_idx is None:
        return text
    return "\n".join(lines[footer_idx + 1:])


def _read_existing(path: Path) -> str:
    """Read a file codebeacon is about to rewrite, tolerating bad encodings.

    A CLAUDE.md saved in a legacy codepage used to abort the whole scan with an
    uncaught ``UnicodeDecodeError`` at its very last step. Undecodable bytes now
    degrade to U+FFFD — and because the degraded text is written straight back,
    the original bytes are copied aside first.
    """
    ...
    if lossy:
        ...
        print(
            ...
            f"replaced; the original was saved to {backup}."
            if backup is not None
            else f"warning: {path} is not valid UTF-8 and could not be backed "
            "up; undecodable bytes were replaced.",
            file=sys.stderr,
        )
    return text
...
def _strip_codebeacon_block(existing: str) -> str:
    ...
    if _BLOCK_START in existing:
        ...
        if _BLOCK_END in after_marker:
            ...
            # Start marker with no end marker — a truncated write, or a user who
            # deleted the closing marker. We cannot bound the managed region by
            # markers any more, so fall back to the footer; with no footer
            # either, keep everything below the marker. A stale block left in
            # place is noise the user can delete; dropping the tail is data loss.
            after = _after_footer(after_marker)
...
def _merge_content(new_content: str, path: Path) -> str:
    ...
    """
    wrapped = f"{_BLOCK_START}\n{defang_model_tokens(new_content).rstrip()}\n{_BLOCK_END}\n"
...
def _merge_rules_content(frontmatter: str, body: str, path: Path) -> str:
    ...
    """
    block = f"{_BLOCK_START}\n{defang_model_tokens(body).rstrip()}\n{_BLOCK_END}\n"
    ...
    if _BLOCK_START in existing and _BLOCK_END in existing:
        ...
        # Start marker but no end (truncated write, or the user deleted the
        # marker). Bound the old block at the generated footer if there is one;
        # otherwise keep everything below the start marker. Discarding the tail
        # here used to destroy hand-written content.
        after_marker = existing[existing.index(_BLOCK_START) + len(_BLOCK_START):]
        tail = _after_footer(after_marker).strip()
