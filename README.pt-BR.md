...
- **`codebeacon watch` mantém o índice ao vivo** — um file-watcher com debounce (`codebeacon watch [path] [--debounce 2.0] [--once] [--exclude PATTERN]`) ressincroniza o grafo sempre que os arquivos-fonte vigiados mudam. Uma rajada de edições — um `git checkout` de 500 arquivos, uma troca de branch — se funde em uma única ressincronização, e o watcher reutiliza exatamente as mesmas regras de ignore do scanner, de modo que escrever o índice nunca o acorda em um loop sobre a sua própria saída `.codebeacon/`. Precisa do novo extra opcional: `pip install 'codebeacon[watch]'` (watchdog).
- **As notas de design se ligam ao grafo de código** — o `codebeacon knowledge` agora escreve suas notas (ADRs, notas de reunião, retros, specs) *dentro* do `beacon.json` quando já existe um índice: uma referência explícita a um caminho de arquivo vira uma aresta `references` confiável, e uma menção de símbolo distintiva (`PaymentService`, nunca um `User` pelado) vira uma aresta `mentions` `AMBIGUOUS` — de modo que um agente que lê o grafo aprende *por que* um service tem a forma que tem. Como o `codebeacon scan` reconstrói o grafo de código apenas a partir do fonte e descarta essa sobrecamada, **rode o `codebeacon knowledge` de novo depois de um scan** para restaurar os links.
- **Ferramenta MCP `beacon_knowledge`** — uma nova ferramenta busca notas por palavra-chave e/ou lista as notas ligadas a um dado nó de código, expondo a trilha de decisões por trás do código diretamente via MCP.
- **Lançador npm para o servidor MCP** — o `@codebeacon/mcp` deixa os clientes MCP iniciarem o servidor do jeito npx-first que eles esperam (`"command": "npx", "args": ["-y", "@codebeacon/mcp"]`). O shim de Node sem dependências resolve um codebeacon funcional via PATH → `uvx` → `pipx run` → `python3 -m codebeacon` e repassa o stdio sem tocar nele. Veja [`npm/README.md`](npm/README.md). (Vem com a 0.7.0; ainda não publicado no npm.)
- **GitHub Action para contexto de PR** — uma action composta comenta em cada pull request com a fatia afetada do seu grafo de conhecimento commitado: os artigos de wiki que a mudança toca, o raio de impacto a montante, e quaisquer arquivos hub de alto impacto que ela edita — uma checagem de deriva de arquitetura para a revisão na era da IA. Requer um índice `.codebeacon/` commitado, `fetch-depth: 0` e `permissions: pull-requests: write`. Veja [`action/README.md`](action/README.md) e [`action/examples/pr-context.yml`](action/examples/pr-context.yml).
- **O CLAUDE.md de workspace fica abaixo de ~200 linhas** — em um workspace multiprojeto, o `CLAUDE.md` raiz agora mantém só a visão geral compartilhada e move o detalhe por projeto para arquivos `.claude/rules/codebeacon-<project>.md` com escopo restrito, cujo frontmatter `paths:` só os carrega quando os arquivos daquele projeto são tocados (seguindo a própria orientação da Anthropic para arquivos de contexto). A saída de projeto único fica inalterada; defina `output.context_map.rules_split: false` para o antigo arquivo monolítico. Linhas de projeto duplicadas também são unificadas.
- **As fixtures de teste são ignoradas por padrão** — `tests/fixtures/`, `test/fixtures/` e `__fixtures__/` em qualquer profundidade agora são ignoradas por padrão, de modo que as entradas de teste sintéticas de um projeto param de injetar rotas e services falsos no grafo (o próprio self-scan do codebeacon havia reportado um `main.py` de fixtures como cinco "rotas"). É a regra de menor precedência, então uma linha `!tests/fixtures/` no `.codebeaconignore` as reinclui, e apontar um scan *para* um diretório de fixtures ainda o coleta.
- **A extração de rotas do Warp agora é de verdade** — as rotas de combinadores de filtro do Warp são realmente extraídas: os segmentos `warp::path!(...)` e `warp::path("x")`, os combinadores de método (`warp::get()` / `post()` / …) e os handlers `.map` / `.and_then` são correlacionados pelo binding que os envolve em rotas inteiras. Limites honestos (detalhados no cabeçalho da query): filtros unidos por `.or(...)` dentro de um mesmo binding colapsam em uma única rota concatenada, e os segmentos de chamada de filtro `warp::path::param()` e os handlers de closure ficam sem resolução.
