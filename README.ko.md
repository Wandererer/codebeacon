...
스캔 후 모든 결과는 `.codebeacon/`에 생성됩니다:

```
.codebeacon/
  beacon.json          ← 전체 지식 그래프 (노드-링크 JSON, 쿼리 가능)
  REPORT.md            ← 갓 노드, 놀라운 연결, 허브 파일
  CLAUDE.md            ← AI 컨텍스트 맵 (프로젝트 루트에도 기록)
  .cursorrules         ← Cursor IDE 컨텍스트
  AGENTS.md            ← OpenAI Agents / Codex 컨텍스트
  wiki/
...
codebeacon scan . --wiki-only             # 재추출 없이 위키만 재생성
...
도구가 성숙해짐에 따라 벤치마크를 공개할 예정입니다. 중간 규모 프로젝트(소스 파일 50~200개)의 초기 스캔은 30초 이내에 완료됩니다. SHA-256 캐싱 덕분에 변경이 없는 코드베이스의 증분 재스캔은 거의 즉각적입니다.

| 코드베이스 | 파일 수 | 노드 | 엣지 | 커뮤니티 | 스캔 시간 |
|-----------|--------|------|------|---------|---------|
| *(출시 예정)* | | | | | |
...
- `--semantic` 플래그(기본 비활성화)는 설정된 LLM API로 코드 발췌문을 전송합니다 — 명시적으로 활성화할 때만 사용됩니다
